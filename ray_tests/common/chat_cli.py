...
async def main(project_dir: Path | None = None):
    """Main entry point."""
    proj_dir = project_dir or Path(__file__).parent.resolve()

    # Create and run CLI
    cli = ChatCLI(proj_dir)
    await cli.run()


def run_cli(project_dir: Path | None = None):
    """Synchronous wrapper for running the chat CLI."""
    try:
