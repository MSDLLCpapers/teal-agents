...
# FIX: Make post field optional to handle health endpoints that only have GET methods
...
class OpenApiPath(BaseModel):
    model_config = ConfigDict(extra="allow")
    post: OpenApiPost | None = None  # ← KEY FIX: Made optional
...
class AgentBuilder:
    ...
    def _get_agent_description(self, agent_name: str) -> str:
        ...
        if response:
            ...
            # Get the main agent path
            agent_path = f"/{AgentBuilder._agent_to_path(agent_name)}"
            if agent_path in response_payload.paths and response_payload.paths[agent_path].post:
                return response_payload.paths[agent_path].post.description
            else:
                # Fallback: try the first path with a POST method
                for path in response_payload.paths.values():
                    if path.post and path.post.description:
                        return path.post.description
                # If still no description found, return a default
