...
class ConversationManager:
        ...
        for directive in directives:
            match directive.op:
                case ContextDirectiveOp.SET:
                    self.upsert_context_item(
                        conversation, directive.key, directive.value
                    )
    ...
    def add_context_item(
        ...
        if item.context_type == ContextType.PERSISTENT:
            self.services_client.add_context_item(
    ...
    def update_context_item(
        ...
        if item.context_type == ContextType.PERSISTENT:
            self.services_client.update_context_item(
                conversation.user_id, item_key, item_value
            )
    ...
    def upsert_context_item(
        self, conversation: Conversation, key: str, value: str
    ) -> None:
        try:
