...
def initialize() -> None:
    ...
    for agent_name in _config.spec.agents:
        ...
        if agent is not None:
            ...
            logger.warning(
                f"Agent {agent_name} failed to initialize and will not be available"
            )

    if failed_agents:
        logger.warning(
            f"The following agents are unavailable: {', '.join(failed_agents)}"
        )
    ...
    if _fallback_agent is None:
        logger.error(
            f"CRITICAL: Fallback agent {_config.spec.fallback_agent} "
            f"could not be initialized!"
        )
        logger.error(
            "Orchestrator will continue but may fail when no agent matches a request"
        )
    ...
    if recipient_chooser_agent is None:
        logger.error(
            f"CRITICAL: Recipient chooser agent {_config.spec.agent_chooser} "
            f"could not be initialized!"
        )
        raise RuntimeError(
            f"Cannot start orchestrator without recipient chooser agent: "
            f"{_config.spec.agent_chooser}"
        )
