...
def resolve_server_auth_headers(server_config: McpServerConfig, user_id: str = "default") -> Dict[str, str]:
    ...
    Args:
        ...
        user_id: User ID for auth lookup (will override Arcade-User-Id)
    ...
    # CRITICAL: Override Arcade-User-Id with runtime user_id for per-user tool discovery
    # This ensures each user sees their own authorized tools, not a static config user
...
class McpPlugin(BasePlugin):
    ...
    def _add_tool_function(self, tool: McpTool):
        ...
        def create_tool_function(captured_tool: McpTool):
            ...
            # NOTE: Don't include server_name here - it's already in the plugin name!
            # SK creates fully qualified names as: plugin_name-function_name
            # So "mcp_arcade" + "-" + "Github_CountStargazers" = "mcp_arcade-Github_CountStargazers"
            raw_name = captured_tool.tool_name
            sanitized_name = "".join(
                c if c.isalnum() or c in "_-" else "_" for c in raw_name
            )
            
            # OpenAI has a 64 character limit on function names (including plugin prefix)
            # Account for plugin name (mcp_{server}) + separator
            plugin_prefix_len = len(f"mcp_{self.server_name}-")
            max_function_name_len = 64 - plugin_prefix_len
            
            logger.info(f"Processing MCP tool: {raw_name} -> {sanitized_name} (len={len(sanitized_name)}, max={max_function_name_len})")
            if len(sanitized_name) > max_function_name_len:
                # Truncate to fit within OpenAI's 64 char limit
                function_name = sanitized_name[:max_function_name_len]
                logger.warning(
                    f"TRUNCATED tool name from {len(sanitized_name)} to {max_function_name_len} chars: "
                    f"{sanitized_name} -> {function_name} (full: mcp_{self.server_name}-{function_name})"
                )
            else:
                function_name = sanitized_name
            
            logger.info(f"Final function name: {function_name} (full qualified: mcp_{self.server_name}-{function_name}, len={plugin_prefix_len + len(function_name)})")
