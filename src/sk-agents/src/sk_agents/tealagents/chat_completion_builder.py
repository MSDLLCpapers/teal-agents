...
class ChatCompletionBuilder:
    def __init__(self, app_config: AppConfig):
        ...
        if cccf_module_name:
            ...
            self.ccc_factory: ChatCompletionFactory | None = (
                ccc_factory_type(
                    app_config
                )
            )
    ...
    def get_chat_completion_for_model(
        ...
        if self.ccc_factory:
            try:
                return (
                    self.ccc_factory.get_chat_completion_for_model_name(
                        model_name,
                        service_id
                    )
                )
        ...
        return (
            self.self_default_cc_factory.get_chat_completion_for_model_name(
                model_name,
                service_id
            )
        )
    ...
    def get_model_type_for_name(self, model_name: str) -> ModelType:
        if self.ccc_factory:
            try:
                return (
                    self.ccc_factory.get_model_type_for_name(
                        model_name
                    )
                )
            except ValueError:
    ...
    def model_supports_structured_output(self, model_name: str) -> bool:
        if self.ccc_factory:
            try:
                return (
                    self.ccc_factory.model_supports_structured_output(
                        model_name
                    )
                )
            except ValueError:
        ...
        return (
            self.self_default_cc_factory.model_supports_structured_output(
                model_name
            )
        )
