...
from semantic_kernel.contents.function_result_content import (
...
class TealAgentsV1Alpha1Handler(BaseHandler):
    ...
    async def _invoke_function(
        kernel,
        fc_content: FunctionCallContent
    ) -> FunctionResultContent:
    ...
    async def authenticate_user(self, token: str) -> str:
        try:
            ...
            raise AuthenticationException(
                message=(
                    f"Unable to authenticate user, exception message: {e}"
                )
            ) from e
    ...
    async def _manage_incoming_task(
        ...
        try:
            ...
            raise Exception(
                f"Unexpected error ocurred while managing incoming task: {e}"
            ) from e
    ...
    async def _manage_hitl_exception(
        ...
        assistant_item = AgentTaskItem(
            ...
            item=MultiModalItem(
                content_type=ContentType.TEXT,
                content="HITL intervention required."
            ),
            request_id=request_id,
            updated=datetime.now(),
    ...
    async def _manage_function_calls(
        ...
        if non_intervention_calls:
            ...
            for result in results:
                chat_history.add_message(
                    result.to_chat_message_content()
                )

        # Handle intervention function calls
        if intervention_calls:
            logger.info(
                f"Intervention required for"
                f"{len(intervention_calls)} function calls."
                )
    ...
    async def prepare_agent_response(
        ...
        await self._manage_agent_response_task(
            agent_task,
            agent_response
        )
        logger.info(
    ...
    async def resume_task(
        ...
        | HitlResponse
        | AsyncIterable[
            TealAgentsResponse | TealAgentsPartialResponse | HitlResponse
        ]
    ):
        ...
        if agent_task is None:
            raise AgentInvokeException(
                f"No agent task found for request ID: {request_id}"
            )
        session_id = agent_task.session_id
        task_id = agent_task.task_id
        ...
        if chat_history is None:
            raise AgentInvokeException(
                f"Chat history not found for request ID: {request_id}"
            )

        TealAgentsV1Alpha1Handler._validate_user_id(
            user_id,
            task_id,
            agent_task
        )
        try:
            ...
            raise Exception(
                f"Agent in resume request is not in Paused state: {e}"
            ) from e
        ...
        if tool_calls_in_task_items is None:
            raise AgentInvokeException(
                f"Pending tool calls no found for request ID: {request_id}"
            )
        _pending_tools = list(
            tool_calls_in_task_items
        )  # [fc for fc in tool_calls_in_task_items]
        pending_tools = [
    ...
    async def invoke(
        ...
        TealAgentsV1Alpha1Handler._build_chat_history(
            agent_task,
            chat_history
        )
    ...
    async def recursion_invoke(
        ...
        chat_history = inputs
        agent_task = await self.state.load_by_request_id(
            request_id
        )
        if not agent_task:
            raise PersistenceLoadError(
                f"Agent task with ID {task_id} not found in state."
            )
        ...
        try:
            ...
            if function_calls:
                await self._manage_function_calls(
                    function_calls,
                    chat_history,
                    kernel
                )
                ...
                if response.content:
                    try:
                        ...
                        extra_data_partial: ExtraDataPartial = (
                            ExtraDataPartial.new_from_json(
                                response.content
                            )
                        )
